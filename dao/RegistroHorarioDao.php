...
class RegistroHorarioDao {
	...
	public function getRegistrosHorario($busca=null) {
		$sql = "SELECT
				    ...
				    dta_registro,
					DATE_FORMAT(hor_entrada, '%H:%i') AS hor_entrada,
					DATE_FORMAT(hor_saida_intervalo, '%H:%i') AS hor_saida_intervalo,
				    DATE_FORMAT(hor_retorno_intervalo, '%H:%i') AS hor_retorno_intervalo,
				    DATE_FORMAT(hor_saida, '%H:%i') AS hor_saida,
				    DATE_FORMAT(hor_extra, '%H:%i') AS hor_extra,
				    dta_lancamento,
				    flg_hora_extra_valida,
				    flg_falta_justificada_valida,
